...
def _list_http_directory(base_url, extension=".parquet"):
    """
...
def download_http_files(base_url, local_dir, download_all=False, file_list=None):
    """
    ...
    if download_all:
        target_files = _list_http_directory(base_url, extension=".parquet")
        if not target_files:
...
def download(conf):
    ...
    download_http_files(conf['ftp_download_url'], conf['gpqt_output_path'], conf['download_all'], conf['dwnld_region_list'])
    t2=time.time()
